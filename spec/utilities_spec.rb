RSpec.describe Martilla::Backup do
  ...
  describe 'formats time durations correctly' do
    it 'for seconds only durations' do
    ...
    it 'for minutes & seconds durations' do
    ...
    it 'for hours & minutes & seconds durations' do
  ...
  describe 'formats time durations correctly using floats as input' do
    it 'for seconds only durations' do
      expect(backup.duration_format(22.824)).to eq('22s')
      expect(backup.duration_format(1.127)).to eq('1s')
      expect(backup.duration_format(59.226)).to eq('59s')
    end

    it 'for minutes & seconds durations' do
      expect(backup.duration_format(192.820)).to eq('3m 12s')
      expect(backup.duration_format(1_044.507)).to eq('17m 24s')
      expect(backup.duration_format(3_599.387)).to eq('59m 59s')
      expect(backup.duration_format(60.811)).to eq('1m 0s')
    end

    it 'for hours & minutes & seconds durations' do
      expect(backup.duration_format(12_292.400)).to eq('3h 24m 52s')
      expect(backup.duration_format(7_044.022)).to eq('1h 57m 24s')
      expect(backup.duration_format(82_599.999)).to eq('22h 56m 39s')
      expect(backup.duration_format(3_600.821)).to eq('1h 0m 0s')
      expect(backup.duration_format(432_00.225)).to eq('12h 0m 0s')
    end
  end
